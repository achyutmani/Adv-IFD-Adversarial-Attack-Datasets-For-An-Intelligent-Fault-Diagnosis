...
def evaluate(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    return epoch_loss / len(iterator), epoch_acc / len(iterator) 
MODEL_SAVE_PATH = os.path.join("/home/mani/Desktop/IJCNN 2022/Bear Fault Detection", 'Fault_CNN3s.pt') # Define Path to save the model 
...
plt.tight_layout()
plt.savefig('Fault_LeNets_CM.png')
#plt.show()
import torchattacks
...
plt.tight_layout()
plt.savefig('Adv_Fault_LeNets_CM_CW.png')